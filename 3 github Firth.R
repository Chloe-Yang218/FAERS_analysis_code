...
dat <- dt_faers_new[,.(primaryid,pt,hlt,drug,combo_category,age,chemcomb,serious,continent_combine,three,HO)]

missing_pct <- mean(is.na(dat$age)) *100;missing_pct
#34.33%

mice_vars <- dat
mice_vars$age <- as.numeric(mice_vars$age)

#Linear models (lm) were used for continuous data, and generalized linear models (glm) for binary data
imp <- mice(mice_vars, seed =123, print =FALSE, m=5) ## m: number of imputations; default = 5
fit <- with(imp, lm( age~ drug + chemcomb + combo_category +three+serious+continent_combine))
est1 <- pool(fit)
...
dt_faers_new_miceage<-setDT(imputed_df)
save(dt_faers_new_miceage,file='dt_faers_new_miceage.Rdata')

dt<-dt_faers_new_miceage[is.na(age),] ##0


####Firth------------------------------------------

knots_locs <- rcs_knots(dt_faers_new_miceage$age, n_knots = 5)
...
for (ae in modified_ae_columns) {
  tryCatch({
    ...
    formula_str <- paste("var_col ~ drug + chemcomb + combo_category + three + serious + continent_combine +", age_term)
    model <- logistf(
