...
dt_faers_new_withage_medical<-dt_faers_new_miceage[primaryid %in% dt_faers_new_medical$primaryid,]

tra_outc_newdate_temp <-dt_faers_new_withage_medical

for(pt_name in ho_pt){
...
original_ae_columns <- names(tra_outc_newdate_temp)[13:34]
modified_ae_columns <- gsub(" ", "_", names(tra_outc_newdate_temp)[13:34])
modified_ae_columns <- gsub("-", "_", modified_ae_columns)

names(tra_outc_newdate_temp)[13:34]<-modified_ae_columns
...
aes <- names(tra_outc_newdate_temp)[13:34] 
drugs<-tra_outc_newdate_temp$drug
